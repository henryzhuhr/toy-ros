...
class MinimalPublisher(Node):    # 创建一个继承自Node的类
    ...
    def __init__(self):        # 构造函数，在类实例化时会自动调用
        ...
        self.publisher_ = self.create_publisher(String, 'topic', 10)    # 创建一个发布者,将数据发布到名为 'topic' 的主题，消息类型是 String 10是发布队列的长度
        timer_period = 0.5  # 单位seconds
        self.timer = self.create_timer(timer_period, self.timer_callback)   # 创建一个定时器，每隔0.5秒调用一次timer_callback 
        self.i = 0
